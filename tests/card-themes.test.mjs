...
assert.ok(compare.includes('.cmp-profile{ display:flex; align-items:center; gap:14px; min-height:132px;'));
assert.ok(compare.includes('.cmp-info{ display:flex; flex:1 1 auto; overflow:hidden; flex-direction:column;'));
assert.ok(compare.includes('profileShowcaseForUserV2(u,116)'),'desktop cards must use the full-body square showcase portrait');
