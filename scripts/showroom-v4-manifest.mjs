...
const runtimeItemV4=entry=>Object.freeze({
...
export function runtimeCatalogV4(manifest) {
  validateManifestV4(manifest);
  return manifest.items.map(runtimeItemV4);
}

function runtimeModuleSourceV4(items){
...
export function runtimeCatalogModuleSourceV4(manifest) { return runtimeModuleSourceV4(runtimeCatalogV4(manifest)); }

export function validateCategoryManifestV4(manifest){
  if(!isRecord(manifest)||manifest.schemaVersion!=='showroom-v4-category/1'||manifest.catalogVersion!=='v4'||manifest.status!=='staging')fail('invalid category manifest header');
  const category=manifest.category;
  if(!V4_CATEGORIES.includes(category)||manifest.assetRoot!=='./assets/showroom-v4')fail('invalid category manifest scope');
  if(!Array.isArray(manifest.items)||manifest.items.length!==12)fail(`${category}: expected 12 category items`);
  const expected=new Set(expectedSlotsV4().filter(slot=>slot.category===category).map(slot=>slot.slot)),ids=new Set(),assets=new Set();
  for(const entry of manifest.items){
    if(!expected.delete(entry.slot)||entry.category!==category||entry.slot!==slotIdV4(category,entry.rarity,entry.variant))fail(`${entry.id}: invalid category slot`);
    if(ids.has(entry.id)||typeof entry.name!=='string'||!entry.name||typeof entry.visual!=='string'||!entry.visual)fail(`${entry.id}: invalid category identity`);ids.add(entry.id);
    if(entry.testOnly!==true||entry.purchasable!==false||entry.persistable!==false||entry.price!==null)fail(`${entry.id}: category safety flags changed`);
    if(entry.qualityTier!==V4_QUALITY_TIERS[entry.rarity]?.id)fail(`${entry.id}: qualityTier mismatch`);
    assertMotionLayer(entry.motionLayer,entry.id);
    if(category==='line_style'){
      if(entry.asset!==null||JSON.stringify(entry.renderSpec)!==JSON.stringify(lineStyleRenderSpecV4(entry.rarity,entry.variant)))fail(`${entry.id}: invalid code-native category item`);
    }else{
      const ext=EXT_BY_CATEGORY[category];if(typeof entry.asset!=='string'||!entry.asset.startsWith(`./assets/showroom-v4/${category}/`)||path.extname(entry.asset).toLowerCase()!==`.${ext}`)fail(`${entry.id}: invalid category asset`);
      if(assets.has(entry.asset))fail(`${entry.id}: duplicate category asset`);assets.add(entry.asset);
      if(entry.mimeType!==MIME_BY_EXT[ext]||!Number.isInteger(entry.width)||!Number.isInteger(entry.height)||!Number.isInteger(entry.byteLength)||!/^[a-f0-9]{64}$/.test(entry.sha256))fail(`${entry.id}: invalid category metadata`);
      if(category==='graph_skin'&&(entry.width!==1536||entry.height!==864||entry.width*9!==entry.height*16))fail(`${entry.id}: graph skin must be exact 1536x864`);
      if(entry.renderSpec!==null)fail(`${entry.id}: raster category renderSpec must be null`);
    }
  }
  if(expected.size)fail(`${category}: missing category slots`);
  return true;
}

export async function registerCategoryAssetsV4({inventory,assetDirectory,category}){
  if(!isRecord(inventory)||inventory.category!==category||!Array.isArray(inventory.items)||inventory.items.length!==12)fail(`${category}: inventory requires exactly 12 items`);
  const items=[];
  for(const source of inventory.items){
    if(source.category!==category)fail(`${source.id}: cross-category inventory item`);
    const ext=EXT_BY_CATEGORY[category],isLine=category==='line_style';
    const relative=isLine?null:canonicalRelativeFile(source.file,category,ext,source.id);
    const metadata=isLine?{width:null,height:null,byteLength:null,sha256:null}:await fileMetadata(path.resolve(assetDirectory,relative),ext,source.id);
    items.push({slot:source.slot,id:source.id,category,rarity:source.rarity,variant:source.variant,name:source.name,visual:source.visual,
      asset:isLine?null:`./assets/showroom-v4/${relative}`,mimeType:isLine?null:MIME_BY_EXT[ext],...metadata,price:null,
      qualityTier:V4_QUALITY_TIERS[source.rarity]?.id,motionLayer:{enabled:false,kind:'none',asset:null,loop:false,durationMs:null,opacity:1,blendMode:'normal'},
      renderSpec:isLine?lineStyleRenderSpecV4(source.rarity,source.variant):null,testOnly:true,purchasable:false,persistable:false});
  }
  const manifest={schemaVersion:'showroom-v4-category/1',catalogVersion:'v4',status:'staging',category,assetRoot:'./assets/showroom-v4',items};
  validateCategoryManifestV4(manifest);return manifest;
}

export async function validateCategoryManifestFilesV4(manifest,assetDirectory){
  validateCategoryManifestV4(manifest);for(const entry of manifest.items){if(entry.asset===null)continue;const relative=entry.asset.slice(`${manifest.assetRoot}/`.length),ext=path.posix.extname(relative).slice(1),metadata=await fileMetadata(path.resolve(assetDirectory,relative),ext,entry.id);if(metadata.byteLength!==entry.byteLength||metadata.sha256!==entry.sha256||metadata.width!==entry.width||metadata.height!==entry.height)fail(`${entry.id}: category asset metadata changed`)}return true;
}

export function runtimeCategoryCatalogModuleSourceV4(manifest){validateCategoryManifestV4(manifest);return runtimeModuleSourceV4(manifest.items.map(runtimeItemV4))}
export function runtimeCategoryCatalogsModuleSourceV4(manifests){const seen=new Set(),items=[];for(const manifest of manifests){validateCategoryManifestV4(manifest);if(seen.has(manifest.category))fail(`${manifest.category}: duplicate category manifest`);seen.add(manifest.category);items.push(...manifest.items.map(runtimeItemV4))}return runtimeModuleSourceV4(items)}
