...
export const RECORD_META_ACHIEVEMENTS = Object.freeze([
  ...
  meta('meta_3m_water_7','3m','수분 기록 일주일','수분을 기록한 날 7일',15,'💧',null),
  meta('meta_3m_water_30','3m','한 달의 물결','수분을 기록한 날 30일',15,'🌊',null),
  ...
  meta('meta_3m_water_history','3m','수분 기록의 역사','수분을 기록한 날 60일',30,'💧','title_water_quartermaster'),
  ...
  meta('meta_6m_never_dry','6m','마르지 않는 기록','수분을 기록한 날 120일',50,'🌊','title_spring_guardian'),
...
export function extractRecordMetaData(records=[]){
  ...
  [...records].sort((a,b)=>String(a.date).localeCompare(String(b.date))).forEach(record=>{
    ...
    const flags={
      ...
      meal:mealCount>0,
      water:Number.isFinite(Number(record.water))&&Number(record.water)>0,
