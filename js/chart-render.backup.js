  ...
  if(showWeeklyBar){
    ...
    canvasBar._chartInstance=new Chart(canvasBar.getContext('2d'),{
      ...
      options:{responsive:true,maintainAspectRatio:false,
        layout:{padding:{right:isMobile?4:70+Y_AXIS_W,left:isMobile?5:70}},
        ...
        scales:{
          x:{...sharedX,ticks:{color:TICK,font:{size:10},maxTicksLimit:8},title:{display:true,text:'날짜',color:TICK,font:{size:10}}},
          y:{grid:{color:GRID},ticks:{color:TICK,font:{size:10},callback:v=>v===0?'0':(v>0?`-${v}`:`+${Math.abs(v)}`)+'kg'},border:{color:'rgba(255,255,255,.15)'},title:{display:true,text:'주간',color:TICK,font:{size:10}},afterFit(s){s.width=Y_AXIS_W;}}
        }
      }
    });
  }
